...
$(document).ready(function () {
  ...
  //   let label = $("#customer_name_update_label");

  //   if (customertype == 3) {

  //     label.contents().filter(function () {
  //       return this.nodeType === 3; // Node type 3 is a text node
  //     }).first().replaceWith('Customer Name');
  ...
  //     label.contents().filter(function() {
  //         return this.nodeType === 3; // Node type 3 is a text node
  //     }).first().replaceWith('Employee Name');
  ...
  //     label.contents().filter(function () {
  //       return this.nodeType === 3; // Node type 3 is a text node
  //     }).first().replaceWith('Guest by Name');
  ...
  // });
    ...
    $("#ctypeid_update").on('change', function () {
        var customertype = $("#ctypeid_update").val();
        let label = $("#customer_name_update_label");
        let labelText = 'Customer Name';
        let isThirdParty = false;
        let isEmployee = false;
        let isGuest = false;
        $("#customer_type_update").val(customertype).change();
        ...
        $("#delivercom_update").prop('disabled', !isThirdParty);
        $("#waiter_update").prop('disabled', isThirdParty);
        $("#tableid_update").prop('disabled', isThirdParty || customertype == 4);
        $("#cookingtime_update").prop('disabled', isThirdParty || customertype == 4);

        $("#nonthirdparty_update").toggle(!isThirdParty);
        $("#thirdparty_update").toggle(isThirdParty);
        $("#tblsecp_update").toggle(!isThirdParty && customertype != 2 && customertype != 4);
        $("#tblsec_update").toggle(!isThirdParty && customertype != 2 && customertype != 4);
        $('#customer_note_update').toggle(isGuest);
    });
